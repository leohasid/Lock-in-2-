  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* All Addictions - Compact Grid */}
        {(phoneAddictions.length > 0 || otherAddictions.length > 0) && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-white mb-4">Your Addictions</h2>
          ...
          phoneAddictions.forEach(addiction => {
            addiction.apps.forEach(app => {
              allApps.push({ app, addictionId: addiction.id });
                ...
                return (
                  <div
                    className={`bg-gray-900 rounded-xl p-6 border-2 text-left transition-all hover:scale-105 ${
                      isBlocked
                        ? "border-red-500/50"
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <h3 className="text-xl font-semibold text-white">Phone & Social Media</h3>
                        <p className="text-xs text-gray-500 mt-1">{daysClean} days tracking</p>
                      </div>
                    </div>
                    <div className="inline-flex flex-col bg-gray-800 rounded-lg px-3 py-2 border border-gray-700 mb-4 max-w-[140px]">
                      <p className="text-[11px] text-gray-400 uppercase tracking-wide">Used today</p>
                      <p className="text-base font-bold text-white">
                        {formatMinutes(totalCurrentUsage)}
                      </p>
                      <p className="text-[10px] text-gray-500">on phone</p>
                    </div>
                    ...
                    <div className="mt-4 flex flex-col gap-2 text-xs text-gray-400">
                      <button
                        ...
                        className="w-full bg-orange-500 hover:bg-orange-600 text-black px-4 py-2 rounded-lg text-sm font-semibold transition-colors"
                      >
                    ...
                    {isBlocked && (
                      <span className="inline-block mt-2 bg-red-600 text-white text-xs px-3 py-1 rounded-full font-semibold">
                        BLOCKED
                      </span>
                    )}
                  </div>
              ...
              {otherAddictions.map((addiction) => {
                ...
                return (
                  <button
                    ...
                    className="bg-gray-900 rounded-xl p-6 border-2 border-gray-800 text-left transition-all hover:scale-105 hover:border-orange-500/50"
                  >
                    <div className="text-4xl mb-3">{getAddictionIcon(addiction)}</div>
                    <h3 className="text-xl font-semibold text-white mb-3">{addiction.name}</h3>
                    <p className="text-sm text-gray-400 mb-4">{daysClean} days clean</p>
                    
                    {/* Live Countdown Timer */}
                    <div className="bg-gray-800 rounded-lg p-4 mb-3">
                      <div className="flex items-center justify-center gap-2 mb-2">
                        <Clock className="w-4 h-4 text-orange-400" />
                        <span className="text-sm text-gray-400 font-semibold">Time Clean</span>
                      </div>
                      <div className="grid grid-cols-4 gap-2 text-center">
                        <div>
                          <div className="text-xl font-bold text-white">{elapsed.days ?? 0}</div>
                          <div className="text-xs text-gray-400">Days</div>
                        </div>
                        <div>
                          <div className="text-xl font-bold text-white">{elapsed.hours ?? 0}</div>
                          <div className="text-xs text-gray-400">Hours</div>
                        </div>
                        <div>
                          <div className="text-xl font-bold text-white">{elapsed.minutes ?? 0}</div>
                          <div className="text-xs text-gray-400">Mins</div>
                        </div>
                        <div>
                          <div className="text-xl font-bold text-white">{elapsed.seconds ?? 0}</div>
